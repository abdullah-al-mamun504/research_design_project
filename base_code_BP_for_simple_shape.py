###########################################
# Backprop Training - Simple Shapes (Fixed)
###########################################
...
def confusion_matrix_bp(model, loader):
...
def evaluate(model, loader):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            total   += y.size(0)
    model.train()                      # ✅ FIX: restore train mode after eval
...
def plot_all(epoch_store, class_names, epochs, best_epoch):
    ...
    fig.suptitle(
        f"Backpropagation Network — Simple Shapes\n"
    ...
    bars = ax5.bar(class_names, final_pc, color=colors,
    ...
    # save_path = r"D:\results\bp_thesis_plots.png"
...
class BPNet(nn.Module):
    ...
    def get_layer_activations(self, x):
        """Returns intermediate activations for dead neuron tracking."""
        x  = x.view(x.size(0), -1)
        h1 = F.relu(self.l1(x))
        h2 = F.relu(self.l2(h1))
        h3 = F.relu(self.l3(h2))
...
def get_layer_loss_contribution(model, x, y, criterion):
    """
    Approximate per-layer loss contribution by measuring how much
    each layer's output magnitude contributes to the final loss.
    Runs a forward pass and records pre-activation norms per layer.
    """
    x   = x.view(x.size(0), -1)
    ...
    # layer contribution approximated as fraction of activation norm
...
def train_model(model, loader, epochs):
    ...
    for epoch in range(1, epochs + 1):
        ...
        layer_loss  = [0.0, 0.0, 0.0, 0.0]
        layer_gnorm = [0.0, 0.0, 0.0, 0.0]
        layer_dead  = [0,   0,   0  ]
        n_batches   = 0
        ...
        for x, y in loader:
            ...
            # ── layer loss contributions ──
            contrib, loss = get_layer_loss_contribution(model, x, y, criterion)
            for i in range(4):
                layer_loss[i] += contrib[i]

            optimizer.zero_grad()
            loss.backward()

            # ── grad norms per layer (after backward, before step) ──
            gnorms = get_layer_grad_norm(model)
            for i in range(4):
                layer_gnorm[i] += gnorms[i]

            optimizer.step()

            # ── dead neurons (no grad needed) ──
            with torch.no_grad():
                acts = model.get_layer_activations(x)
                for i in range(3):
                    layer_dead[i] += count_dead_neurons(acts[i])
    ...
    print(f"Architecture   : 784 → 500 → 500 → 500 → 5")
