###########################################
# Experiment 3: FF Training on MNIST (Fixed)
###########################################
...
# ✅ FIX 1: neg_labels — offset method, guaranteed different label
def neg_labels(y):
...
def plot_all(epoch_store, class_names, epochs, best_epoch):
    ...
    fig.suptitle(
        f"Forward-Forward Network — MNIST\n"
    ...
    ax3.axhline(0.3, color='gray',  linestyle=':', linewidth=1,
                alpha=0.6, label='Gap=0.3 (small)')
    ax3.axhline(0.8, color='black', linestyle=':', linewidth=1,
                alpha=0.6, label='Gap=0.8 (clear)')
    ...
    # save_path = r"D:\results\mnist_ff_thesis_plots.png"
...
class FFLayer(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.opt    = torch.optim.Adam(self.parameters(), lr=lr)  # ✅ FIX 3: user input lr
    ...
    def train_step(self, xp, xn):
        ...
        loss = (F.softplus(-gp + threshold) +          # ✅ FIX 3: user input threshold
        ...
        # ✅ FIX 2: normalize before passing to next layer
...
class FFNet:
    def __init__(self):
        # ✅ FIX 6: 500 units per layer (was 200)
        self.layers = [
            FFLayer(784, 500),
            FFLayer(500, 500),
            FFLayer(500, 500),
    ...
    # ✅ FIX 5: evaluate() method added to class
    ...
    def predict(self, x):
        ...
        for c in range(NUM_CLASSES):
            ...
            for layer in self.layers:
                h  = layer.forward(h)
                g += (h ** 2).mean(dim=1)
                # ✅ FIX 2: normalize between layers during inference
    ...
    def train(self, loader, epochs):
        ...
        for epoch in range(1, epochs + 1):
            ...
            # ✅ FIX 4: test accuracy evaluated every epoch
